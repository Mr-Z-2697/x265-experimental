...
#define CHECK(expr, msg) check_failed |= _confirm(param, expr, msg)
    ...
    CHECK(param->rc.aqMode < X265_AQ_NONE || (X265_AQ_EDGE_BIASED < param->rc.aqMode && param->rc.aqMode != X265_AQ_VARIANCE_BIASED && param->rc.aqMode != X265_AQ_VARIANCE_AUTO_MIN && param->rc.aqMode != X265_AQ_VARIANCE_AUTO_MIN_BIASED),
...
#define BOOL(param, cliopt) \
    ...
    if (p->bEnableHME)
    {
        s += snprintf(s, bufSize - (s - buf), " hme-level-0,1,2=%d,%d,%d", p->hmeSearchMethod[0], p->hmeSearchMethod[1], p->hmeSearchMethod[2]);
        s += snprintf(s, bufSize - (s - buf), " hme-range-L0,L1,L2=%d,%d,%d", p->hmeRange[0], p->hmeRange[1], p->hmeRange[2]);
    }
    else
    {
