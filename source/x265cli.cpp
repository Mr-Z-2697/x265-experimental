...
#define H1 if (level >= X265_LOG_DEBUG) printf
        ...
        H0("                                 vq1~vq3(vq1 if no trailing number(i.e. --tune vq == --tune vq1)), none\n");
        ...
        H0("   --[no-]rd-refine              Enable QP-based RD refinement for rd levels 5 and 6. 2: disable skip in recodeCU. Default %s\n", OPT(param->rdRefine));
        ...
        H0("   --aq-bias-strength <float>    Sets dark bias strength in AQ modes that use it. This acts as a factor by which the bias_strength which was the same as aq-strength is multiplied. Default %.2f\n", param->rc.aqBiasStrength);
...
#undef H1
    ...
    void CLIOptions::printStatus(uint32_t frameNum)
    {
        ...
        if (framesToBeEncoded)
        {
            ...
            snprintf(buf, sizeof(buf), "x265 [%.1f%%] %d/%d frames, %.*f fps, %.*f kb/s, %d:%02d:%02d, %.*f %sB, -%d:%02d:%02d, ~%.*f %sB",
